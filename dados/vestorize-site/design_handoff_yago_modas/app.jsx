...
const ACCENTS = {
  "#C8A84B": { soft: "#ddc275", deep: "#8f7327", rgb: "200,168,75" },   // Dourado
  "#C0824B": { soft: "#d7a06d", deep: "#8a5a2e", rgb: "192,130,75" },   // Bronze
  "#B9BEC6": { soft: "#d6dae0", deep: "#888d96", rgb: "185,190,198" },  // Platina
};
...
function RoleGate({ serif, onClient }) {
  const goManager = () => { window.location.href = "Gerenciador Yago Modas.html"; };
  return (
...
function App() {
  ...
  const [role, setRole] = React.useState(null);   // null | 'cliente'

  const [view, setView] = React.useState("home");   // home | browse
  ...
  // reage a edições feitas no Gerenciador (mesma aba ou outra aba)
  React.useEffect(() => {
  ...
  // persistir sacolinha no aparelho
  React.useEffect(() => {
  ...
  const rootStyle = (() => {
    const a = ACCENTS[t.accent] || ACCENTS["#C8A84B"];
    return {
      "--accent": t.accent, "--accent-soft": a.soft, "--accent-deep": a.deep,
      "--accent-rgb": a.rgb, "--r": t.radius + "px",
    };
  })();
  ...
  return (
    <div className="cat" style={rootStyle}>
      ...
      <Header query={query} setQuery={setQuery} cartCount={cartCount} bump={bump}
        onCart={() => setCartOpen(true)} serif={t.serifLogo} />
      ...
      <TweaksPanel>
        ...
        <TweakColor label="Cor de destaque" value={t.accent}
          options={["#C8A84B", "#C0824B", "#B9BEC6"]}
          onChange={(v) => setTweak("accent", v)} />
        <TweakToggle label="Logo serifada" value={t.serifLogo}
          onChange={(v) => setTweak("serifLogo", v)} />
        <TweakSection label="Cards" />
        <TweakToggle label="Mostrar tamanhos no card" value={t.showSizes}
          onChange={(v) => setTweak("showSizes", v)} />
        <TweakSlider label="Cantos" value={t.radius} min={6} max={22} step={1} unit="px"
...
/* ---- montagem dentro da moldura iOS + escala responsiva ---- */
function Stage() {
